...
## Migration smoke (2026-05-18)

Applied migration `0061_connector_broken_notify` to live staging and ran a controlled trigger smoke.

- **Project**: `amjvkukmdpmimelazpyw` (bbc-staging, us-west-2)
- **Tenant tested**: `86e50879-2bca-48b2-aaf4-d0f756a38d5b` (slug `oscartry`) — 1 admin (user `2891990a-64a5-4ec8-bcbb-531bfaac88bb`). No tenant slugged `8azi` exists on staging; `oscartry` is the user's own dogfood tenant per project memory.
- **Connector row used**: `e14babd1-5083-4dda-8bf9-d08ff1fc0f67` — `connector_id = github`, original state `(last_sync_status=NULL, last_sync_at=NULL, last_sync_error=NULL)`. (A second identical github connector exists on the same tenant — `8ca4ae34-…` — untouched by the smoke.)
- **Pre-smoke inbox count** (`source_kind='connector'`, tenant = oscartry): **0**

Trigger / constraints verified post-apply:
- `notify_connector_broken_trig` on `tenant_connectors` → `notify_connector_broken` exists exactly once.
- `inbox_items_source_kind_check` widened to include `connector`.
- `inbox_items_kind_check` added with the planned 5-value union.

Smoke fires:

| # | Transition | Trigger fires? | Inbox count after | Result |
|---|---|---|---|---|
| F | NULL → auth_expired | yes | 1 | PASS — exactly 1 row per admin, `kind=connector_auth_expired`, `source_kind=connector`, title `"github connection expired — reconnect required"`, body = simulated 401 text. |
| G | auth_expired → auth_expired (no-op UPDATE) | no | 1 (unchanged) | PASS — WHEN clause (`OLD IS DISTINCT FROM NEW` + OLD ∈ {NULL,'ok'}) suppresses dup. |
| H | auth_expired → ok → auth_expired | yes (on ok→auth_expired only) | 2 | PASS — recovery + re-break re-notifies. Confirms `OLD='ok'` is treated as a healthy transition source. |

Cleanup verified:
- Connector row restored to `(NULL, NULL, NULL)`. The intermediate `auth_expired → NULL` reset does not fire the trigger (NEW not in {auth_expired, error}), so no spurious notifications were generated by the restore.
- Both smoke `inbox_items` rows deleted; count back to **0** (= `before_count`).

Advisors after apply:
- Security: **2 new WARN** advisors introduced by 0061, both for `public.notify_connector_broken()` — `anon_security_definer_function_executable` + `authenticated_security_definer_function_executable`. The function is a trigger function, not intended as an RPC; recommended follow-up is `REVOKE EXECUTE ON FUNCTION public.notify_connector_broken() FROM PUBLIC, anon, authenticated;` in a follow-up patch migration. Functionally harmless (no RPC code path uses it) but cleanly fixable.
- Performance: no new lints attributable to 0061. Pre-existing `tenant_connectors` lints (unindexed FKs on `installed_by`, `tenant_id_external_account_id`; `auth_rls_initplan` on existing RLS policies) were already present.

Final state: trigger live, constraints in place, test data fully removed, connector row at pre-smoke state, no new perf regressions, two pre-existing-style SECURITY DEFINER warnings to clean up in a follow-up.
