...
describe("GalleryClient", () => {
  ...
  it("does not render the Ask BBC eyebrow (moved to /home)", () => {
    render(<GalleryClient templates={FIXTURES} />);
    expect(screen.queryByText(/ask bbc/i)).toBeNull();
  });
  it("does not render the recent runs across studios section", () => {
    render(<GalleryClient templates={FIXTURES} />);
    expect(screen.queryByText(/recent runs across all studios/i)).toBeNull();
  });
});
