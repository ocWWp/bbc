...
export default async function ApiKeysPage({ searchParams }: { searchParams: SearchParams }) {
  const { ok, error, token, name: tokenName, scope: tokenScope } = await searchParams;
  ...
  const { data: keys } = await sb
    .from("api_keys")
    .select("id, key_id, scope, name, created_at, last_used_at, revoked_at")
    ...
    <main style={{ maxWidth: 880, margin: "32px auto", padding: 24 }}>
      ...
      {token && (
        ...
        >
          ...
          <p>
            Key <strong>{tokenName}</strong> ({tokenScope}) was created. The plaintext is shown
            below ONCE. After you navigate away, only the bcrypt hash is stored on the server.
            If you lose this token, revoke it and create a new one.
          </p>
          <pre
            style={{
              padding: 12,
