...
import {
  deactivateFinanceStudioOverride,
  listActiveFinanceOverrides,
  proposeFinanceOverride,
  runFinanceWorkflow,
  saveFinanceStudioTemplateOverride,
  type CitedMemoryRef,
} from "./actions";

type Props = {
  templates: ClientFinanceTemplate[];
};

type Stage =
  | { kind: "idle" }
  | { kind: "configuring"; template: ClientFinanceTemplate; task: string }
  | { kind: "running"; template: ClientFinanceTemplate; task: string }
  | {
      kind: "reviewing";
      template: ClientFinanceTemplate;
      task: string;
      blocks: OutputBlock[];
...
export default function FinanceStudioClient({ templates }: Props) {
  ...
  const [selected, setSelected] = useState<ClientFinanceTemplate | null>(null);
  const [inputs, setInputs] = useState<Record<string, string>>({});
  const [pending, startTransition] = useTransition();

  const pickTemplate = (t: ClientFinanceTemplate) => {
    setSelected(t);
    setInputs(
  ...
  const run = () => {
    ...
    startTransition(async () => {
      const res = await runFinanceWorkflow(selected.id, task, inputs);
      if (!res.ok) {
    ...
    <div className="space-y-8">
      <section>
        ...
        <textarea
          ...
          placeholder="e.g. The board meeting is Thursday and I need the Q3 financials section written up from our actuals."
          value={task}
