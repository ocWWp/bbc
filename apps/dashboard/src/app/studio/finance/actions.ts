...
/**
 * Finance Studio — Loop 2 role agent. Board financials, budget memos,
 * investor updates, expense policies, runway analyses. Same shape as the
 ...
 * "finance:*"; runs share the `studio_runs` table and overrides share
 * `studio_template_overrides` (keyed by template_id, so the "finance:*"
 * namespace isolates them).
 *
 * Finance output is the `doc` OutputBlock kind (structured documents), not
 * marketing's platform cards. The hard rule baked into every template +
 * the run system prompt: never invent a financial figure.
 */
...
const EMIT_OUTPUT_TOOL = {
  ...
  description:
    "Return the generated finance document as a single OutputBlock of kind 'doc' with props { title, doc_type, body_markdown }.",
...
export type RunFinanceWorkflowResult =
  | {
      ok: true;
      runId: string;
      blocks: OutputBlock[];
...
export async function runFinanceWorkflow(
...
): Promise<RunFinanceWorkflowResult> {
  ...
  const template = getFinanceTemplate(templateId);
  ...
  const system =
    "You are BBC's finance documents generator. Produce board financials, budget memos, investor updates, expense policies, and runway analyses grounded in the team's decisions, vendors, and recorded metrics. HARD RULE: never invent a financial figure -- use only numbers present in the brain context or the user's inputs; a number you don't have goes under 'Open questions', never estimated into the document. Cite real memory ids only -- never invent them. Return via the emit_output_blocks tool only.";

  const resolvedModel = await resolveLlmModel(RUN_MODEL_FALLBACK);
  console.info(
    `studio.runFinanceWorkflow: tenant=${tenantId} template=${templateId} cost=${costAttribution} model=${resolvedModel.model_id} (${resolvedModel.source})`,
  );
...
// finance templates use the "finance:*" namespace they never collide with
// the other roles. System prompts and the "what an override is" framing are
// finance-specific.
// ----------------------------------------------------------------------------
...
const PROPOSE_OVERRIDE_TOOL = {
  ...
  description:
    "Convert the finance lead's correction message into one structured override rule. Pick the kind that best fits and return a concrete, actionable value object.",
  input_schema: {
    ...
    properties: {
      kind: {
        ...
        description:
          "add_constraint: add a 'never/always' rule. replace_section: swap a section of the prompt. add_example: pin a concrete reference (e.g. a board memo to model). forbid_pattern: ban a phrase or pattern.",
      },
      value: {
        type: "object",
        description:
      ...
      summary: {
        type: "string",
        description: "One sentence (max 280 chars) the finance lead will see on the chip.",
      },
    },
...
export async function proposeFinanceOverride(
  templateId: string,
  message: string,
  ...
  const system =
    "You convert a finance lead's correction into a structured prompt override for a finance-documents workflow (board financials, budget memos, investor updates, expense policies, runway analyses). Pick the most specific override kind. Keep value fields concrete (a constraint sentence, a pattern, an example). Do not invent rules the finance lead did not state. Never duplicate an existing override.";
  ...
  const userMessage = [
    ...
    existingBlurb,
    "",
    `Finance lead correction: ${trimmed}`,
    "",
    "Return one override via the propose_override tool.",
  ].join("\n");

  console.info(
    `studio.proposeFinanceOverride: tenant=${a.actor.tenant_id} template=${templateId} cost=${costAttribution}`,
  );
  let resp: Anthropic.Messages.Message;
  try {
    resp = await client.messages.create({
...
export async function saveFinanceStudioTemplateOverride(
...
export async function listActiveFinanceOverrides(
  templateId: string,
...
export async function deactivateFinanceStudioOverride(
  overrideId: string,
