...
import {
  deactivateLegalStudioOverride,
  listActiveLegalOverrides,
  proposeLegalOverride,
  runLegalWorkflow,
  saveLegalStudioTemplateOverride,
  type CitedMemoryRef,
} from "./actions";

type Props = {
  templates: ClientLegalTemplate[];
};

type Stage =
  | { kind: "idle" }
  | { kind: "configuring"; template: ClientLegalTemplate; task: string }
  | { kind: "running"; template: ClientLegalTemplate; task: string }
  | {
      kind: "reviewing";
      template: ClientLegalTemplate;
      task: string;
      blocks: OutputBlock[];
...
export default function LegalStudioClient({ templates }: Props) {
  ...
  const [selected, setSelected] = useState<ClientLegalTemplate | null>(null);
  const [inputs, setInputs] = useState<Record<string, string>>({});
  const [pending, startTransition] = useTransition();

  const pickTemplate = (t: ClientLegalTemplate) => {
    setSelected(t);
    setInputs(
  ...
  const run = () => {
    ...
    startTransition(async () => {
      const res = await runLegalWorkflow(selected.id, task, inputs);
      if (!res.ok) {
    ...
    <div className="space-y-8">
      <section>
        ...
        <textarea
          ...
          placeholder="e.g. We're bringing on a freelance designer next week and need a contractor agreement."
          value={task}
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <TriageChip level={t.triageLevel} />
                </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <TriageChip level={selected.triageLevel} />
              </div>
              <div className="text-xs text-muted-foreground mt-1">{selected.hint}</div>
              <div className="text-xs text-muted-foreground mt-2 rounded-md border border-border bg-muted/40 px-2.5 py-1.5">
                {selected.triageNote}
              </div>
            </div>
            <ActiveOverridesPill
      ...
      <header className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs text-muted-foreground uppercase tracking-wide flex items-center gap-2">
            ...
            <TriageChip level={template.triageLevel} />
            <ActiveOverridesPill
