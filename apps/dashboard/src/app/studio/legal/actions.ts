...
import "@/lib/studio/legal-templates"; // side-effect: register the 5 templates
import { getLegalTemplate } from "@/lib/studio/legal-templates/registry";
import type { OverrideRule } from "@/lib/studio/legal-templates/types";
...
/**
 * Legal Studio — Loop 2 role agent. NDAs, contractor agreements, IP
 * assignments, ToS/privacy policies, employment terms. Same shape as the
 * Engineering Studio: no LLM router (the user picks the template directly),
 * plus the conversational override flow. template_id is namespaced
 * "legal:*"; runs share the `studio_runs` table and overrides share
 * `studio_template_overrides` (keyed by template_id, so the "legal:*"
 * namespace isolates them).
 *
 * Legal output is the `doc` OutputBlock kind. THE HARD CONSTRAINT
 * (UI-SPEC §2): this is a DRAFTING ASSISTANT, never a legal advisor. The
 * run system prompt, every template's LEGAL_REVIEW_CONTRACT, the persistent
 * LegalDisclaimerBanner, and the per-doc-type triage chip all enforce it.
 */
...
const EMIT_OUTPUT_TOOL = {
  ...
  description:
    "Return the generated legal document as a single OutputBlock of kind 'doc' with props { title, doc_type, body_markdown }.",
  input_schema: EMIT_OUTPUT_TOOL_INPUT_SCHEMA,
  cache_control: { type: "ephemeral" as const },
};

// Legal inputs can carry pasted context (product/data descriptions, agreed
// terms), so the per-value cap is roomier than the marketing/eng default.
const inputsRecordSchema = z.record(z.string(), z.string().max(INPUT_MAX_LEN.legal));
...
export type RunLegalWorkflowResult =
  | {
      ok: true;
      runId: string;
      blocks: OutputBlock[];
...
export async function runLegalWorkflow(
  templateId: string,
  task: string,
  inputs: Record<string, string>,
): Promise<RunLegalWorkflowResult> {
  ...
  if (trimmed.length > TASK_MAX_LEN.legal) {
    return { ok: false, error: `Task too long -- keep it under ${TASK_MAX_LEN.legal} characters.` };
  }
  ...
  const system =
    "You are BBC's legal documents generator -- a DRAFTING ASSISTANT, never a legal advisor. Produce NDAs, contractor agreements, IP assignments, ToS/privacy policies, and employment terms anchored to standard, well-trodden forms. HARD RULES: every output is a draft for attorney review, never final; never assert a document is enforceable, binding, valid, compliant, or safe to use; never give legal advice on the user's specific situation; every document opens with the line 'DRAFT -- not legal advice. For attorney review before use.' and ends with a '## Before you use this' section; flag every jurisdiction-dependent clause -- you do not know the user's jurisdiction. Cite real memory ids only -- never invent them. Return via the emit_output_blocks tool only.";

  const resolvedModel = await resolveLlmModel(RUN_MODEL_FALLBACK);
  console.info(
    `studio.runLegalWorkflow: tenant=${tenantId} template=${templateId} cost=${costAttribution} model=${resolvedModel.model_id} (${resolvedModel.source})`,
  );
...
// storage table is shared between studios, keyed by template_id; because
// legal templates use the "legal:*" namespace they never collide with the
// other roles. System prompts and the "what an override is" framing are
// legal-specific.
// ----------------------------------------------------------------------------
  ...
  input_schema: {
    ...
    properties: {
      kind: {
        ...
        description:
          "add_constraint: add a 'never/always' rule. replace_section: swap a section of the prompt. add_example: pin a concrete reference (e.g. a clause to model). forbid_pattern: ban a phrase or pattern.",
      },
      value: {
        type: "object",
        description:
...
export async function proposeLegalOverride(
  templateId: string,
  message: string,
  ...
  const system =
    "You convert a user's correction into a structured prompt override for a legal-documents drafting workflow (NDAs, contractor agreements, IP assignments, ToS/privacy, employment terms). Pick the most specific override kind. Keep value fields concrete (a constraint sentence, a pattern, an example). Do not invent rules the user did not state. Never duplicate an existing override. Never produce an override that weakens the 'drafting assistant, not legal advice' framing -- that constraint is non-negotiable.";
  ...
  console.info(
    `studio.proposeLegalOverride: tenant=${a.actor.tenant_id} template=${templateId} cost=${costAttribution}`,
  );
  let resp: Anthropic.Messages.Message;
  try {
    resp = await client.messages.create({
...
export async function saveLegalStudioTemplateOverride(
  ...
  const template = getLegalTemplate(input.templateId);
...
export async function listActiveLegalOverrides(
  templateId: string,
...
export async function deactivateLegalStudioOverride(
  overrideId: string,
