...
import {
  ...
  proposeSupportOverride,
  runSupportWorkflow,
  saveSupportStudioTemplateOverride,
  type CitedMemoryRef,
} from "./actions";

export type RecentSupportRun = {
  id: string;
  templateId: string;
  task: string;
...
type Props = {
  templates: ClientSupportTemplate[];
  recentRuns: RecentSupportRun[];
};

type Stage =
  | { kind: "idle" }
  | { kind: "configuring"; template: ClientSupportTemplate; task: string }
  | { kind: "running"; template: ClientSupportTemplate; task: string }
  | {
      kind: "reviewing";
      template: ClientSupportTemplate;
      task: string;
      blocks: OutputBlock[];
...
export default function SupportStudioClient({ templates, recentRuns }: Props) {
  const [stage, setStage] = useState<Stage>({ kind: "idle" });
  const [task, setTask] = useState("");
  const [selected, setSelected] = useState<ClientSupportTemplate | null>(null);
  const [inputs, setInputs] = useState<Record<string, string>>({});
  const [pending, startTransition] = useTransition();

  const pickTemplate = (t: ClientSupportTemplate) => {
    setSelected(t);
    setInputs(
      Object.fromEntries(
  ...
  const run = () => {
    ...
    startTransition(async () => {
      const res = await runSupportWorkflow(selected.id, task, inputs);
      if (!res.ok) {
  ...
  if (templates.length === 0) {
    return (
      <section className="rounded-lg border border-dashed border-border p-8 text-center">
        <div className="font-medium">No support workflows yet</div>
    ...
    <div className="space-y-8">
      <section>
        ...
        <textarea
          ...
          placeholder="e.g. Paying customer is asking why our pricing went up last month."
          value={task}
      ...
      <section>
        ...
        {task.trim().length < 8 && (
          <p className="mt-2 text-xs text-muted-foreground">
            Describe the situation first, then pick a workflow.
          </p>
        )}
      </section>
        ...
        <section className="rounded-lg border border-border p-5 space-y-4">
            ...
            <div key={fi.id}>
              ...
              {fi.kind === "select" && fi.options ? (
                <select
                  ...
                  disabled={pending}
                >
                  {!fi.required && <option value="">(none)</option>}
                  {fi.options.map((opt) => (
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button onClick={run} disabled={pending || !canRun(selected, inputs, task)}>
              {pending ? "Generating…" : "Draft reply"}
            </Button>
            <button
              type="button"
              onClick={reset}
