 ...
 * Studio: no LLM template router (the user picks the template directly).
 * Writes into the same `studio_runs` table -- template_id is namespaced "eng:*"
 * so it doesn't clash with marketing. Override storage is also shared: the
 * `studio_template_overrides` table is keyed by (tenant_id, template_id), so
 * eng:* templates have their own override set, isolated from marketing's.
 *
...
//
// Mirrors the J.14/J.15 flow in src/app/studio/marketing/actions.ts. The
// override storage table is shared between studios, keyed by template_id;
// because engineering templates use the "eng:*" namespace they never collide
// with marketing's. System prompts and the "what an override is" framing are
...
const PROPOSE_OVERRIDE_TOOL = {
  ...
  description:
    "Convert the engineer's correction message into one structured override rule. Pick the kind that best fits and return a concrete, actionable value object.",
  input_schema: {
    ...
    properties: {
      kind: {
        ...
        description:
          "add_constraint: add a 'never/always' rule. replace_section: swap a section of the prompt. add_example: pin a concrete reference (e.g. an ADR to model). forbid_pattern: ban a phrase or pattern.",
      },
      value: {
        type: "object",
        description:
      ...
      summary: {
        type: "string",
        description: "One sentence (max 280 chars) the engineer will see on the chip.",
      },
    },
...
export async function proposeEngOverride(
  templateId: string,
  message: string,
  ...
  const system =
    "You convert an engineer's correction into a structured prompt override for an engineering-documents workflow (ADR drafts, vendor swap memos, tech-debt reviews). Pick the most specific override kind. Keep value fields concrete (a constraint sentence, a pattern, an example). Do not invent rules the engineer did not state. Never duplicate an existing override.";
  ...
  const userMessage = [
    ...
    existingBlurb,
    "",
    `Engineer correction: ${trimmed}`,
    "",
    "Return one override via the propose_override tool.",
  ].join("\n");

  console.info(
    `studio.proposeEngOverride: tenant=${a.actor.tenant_id} template=${templateId} cost=${costAttribution}`,
  );
  let resp: Anthropic.Messages.Message;
  try {
    resp = await client.messages.create({
...
export async function saveEngStudioTemplateOverride(
  ...
  const template = getEngTemplate(input.templateId);
...
export async function listActiveEngOverrides(
  templateId: string,
...
export async function deactivateEngStudioOverride(
  overrideId: string,
