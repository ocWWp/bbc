...
import type { OverrideRule } from "@/lib/studio/designer-templates/types";
 ...
 * entries, UI copy passes. Same shape as the Engineering Studio: no LLM
 * router (the user picks the template directly), plus the conversational
 * override flow. template_id is namespaced "design:*"; runs share the
 * `studio_runs` table and overrides share `studio_template_overrides`
 * (keyed by template_id, so the "design:*" namespace isolates them).
 */
...
const inputsRecordSchema = z.record(z.string(), z.string().max(INPUT_MAX_LEN.designer));
...
export type RunDesignerWorkflowResult =
  | {
      ok: true;
      runId: string;
      blocks: OutputBlock[];
...
export async function runDesignerWorkflow(
  templateId: string,
  task: string,
  inputs: Record<string, string>,
): Promise<RunDesignerWorkflowResult> {
  ...
  const template = getDesignerTemplate(templateId);
  ...
  if (trimmed.length > TASK_MAX_LEN.designer) {
    return { ok: false, error: `Task too long -- keep it under ${TASK_MAX_LEN.designer} characters.` };
  }
  ...
  const system =
    "You are BBC's design documents generator. Produce visual specs, brand guideline entries, and UI copy passes grounded in the team's voice memory and product positioning. Cite real memory ids only -- never invent them. Return via the emit_output_blocks tool only.";

  const resolvedModel = await resolveLlmModel(RUN_MODEL_FALLBACK);
  console.info(
    `studio.runDesignerWorkflow: tenant=${tenantId} template=${templateId} cost=${costAttribution} model=${resolvedModel.model_id} (${resolvedModel.source})`,
  );
  ...
  revalidatePath("/studio/designer");
...
// designer templates use the "design:*" namespace they never collide with
// marketing or engineering. System prompts and the "what an override is"
// framing are designer-specific.
// ----------------------------------------------------------------------------
...
const PROPOSE_OVERRIDE_TOOL = {
  ...
  description:
    "Convert the designer's correction message into one structured override rule. Pick the kind that best fits and return a concrete, actionable value object.",
  input_schema: {
    ...
    properties: {
      kind: {
        ...
        description:
          "add_constraint: add a 'never/always' rule. replace_section: swap a section of the prompt. add_example: pin a concrete reference (e.g. a spec to model). forbid_pattern: ban a phrase or pattern.",
      },
      value: {
        type: "object",
        description:
      ...
      summary: {
        type: "string",
        description: "One sentence (max 280 chars) the designer will see on the chip.",
      },
    },
...
export async function proposeDesignerOverride(
  templateId: string,
  message: string,
  ...
  const system =
    "You convert a designer's correction into a structured prompt override for a design-documents workflow (visual specs, brand guideline entries, UI copy passes). Pick the most specific override kind. Keep value fields concrete (a constraint sentence, a pattern, an example). Do not invent rules the designer did not state. Never duplicate an existing override.";
  ...
  const userMessage = [
    ...
    existingBlurb,
    "",
    `Designer correction: ${trimmed}`,
    "",
    "Return one override via the propose_override tool.",
  ].join("\n");

  console.info(
    `studio.proposeDesignerOverride: tenant=${a.actor.tenant_id} template=${templateId} cost=${costAttribution}`,
  );
  let resp: Anthropic.Messages.Message;
  try {
    resp = await client.messages.create({
...
export async function saveDesignerStudioTemplateOverride(
  ...
  const template = getDesignerTemplate(input.templateId);
...
export async function listActiveDesignerOverrides(
  templateId: string,
...
export async function deactivateDesignerStudioOverride(
  overrideId: string,
