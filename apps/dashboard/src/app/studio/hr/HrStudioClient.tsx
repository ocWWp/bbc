...
import {
  deactivateHrStudioOverride,
  listActiveHrOverrides,
  proposeHrOverride,
  runHrWorkflow,
  saveHrStudioTemplateOverride,
  type CitedMemoryRef,
} from "./actions";

type Props = {
  templates: ClientHrTemplate[];
};

type Stage =
  | { kind: "idle" }
  | { kind: "configuring"; template: ClientHrTemplate; task: string }
  | { kind: "running"; template: ClientHrTemplate; task: string }
  | {
      kind: "reviewing";
      template: ClientHrTemplate;
      task: string;
      blocks: OutputBlock[];
...
export default function HrStudioClient({ templates }: Props) {
  ...
  const [selected, setSelected] = useState<ClientHrTemplate | null>(null);
  const [inputs, setInputs] = useState<Record<string, string>>({});
  const [pending, startTransition] = useTransition();

  const pickTemplate = (t: ClientHrTemplate) => {
    setSelected(t);
    setInputs(
  ...
  const run = () => {
    ...
    startTransition(async () => {
      const res = await runHrWorkflow(selected.id, task, inputs);
      if (!res.ok) {
    ...
    <div className="space-y-8">
      <section>
        ...
        <textarea
          ...
          placeholder="e.g. We're opening our first product designer role and I need the job description."
          value={task}
