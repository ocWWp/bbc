...
const inputsRecordSchema = z.record(z.string(), z.string().max(INPUT_MAX_LEN.hr));
...
export async function runHrWorkflow(
  ...
  if (trimmed.length > TASK_MAX_LEN.hr) {
    return { ok: false, error: `Task too long -- keep it under ${TASK_MAX_LEN.hr} characters.` };
  }
